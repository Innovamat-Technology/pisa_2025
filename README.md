...
An additional [seven-country Latin American analysis](results/python/latam/README.md) repeats the 2022–2025 mathematics quartile comparisons for Argentina, Brazil, Chile, Colombia, Mexico, Peru and Uruguay. It includes levels, changes, Q4−Q1 gaps, confidence intervals, an Excel workbook and figures. Its harmonized indices are recalibrated on those seven countries jointly across the two cycles; they are a regional extension of the method. Run `python code/python/scripts/14_latam_quartiles.py` with the original 2022 and 2025 SPSS files available. This optional analysis writes to `results/python/latam/` and is separate from the main OECD workflow.

Reports, workbooks and figures are also available in [Spanish](results/python/latam/es/README.md) and [English](results/python/latam/en/README.md). The [historical series](results/python/latam/history/en/README.md) extends the **level of the Q4−Q1 mathematics gap** to 2015, comparing published and harmonized ESCS and HOMEPOS in every cycle. Argentina starts in 2018 because its 2015 national sample is not comparable. Historical harmonization uses one model with 13 common items across all four cycles, so its 2022 and 2025 harmonized estimates need not equal the two-cycle, 16-item estimates. Run optional scripts `15_latam_history.py` and `16_latam_reports.py` to calculate and render these outputs. The [language and download index](results/python/latam/index.md) links all versions.
