...
in_sample = keep & ~d.cnt.isin(C.EXCLUDE).to_numpy()
rows = np.flatnonzero(keep)[np.argsort(-in_sample[keep].astype(int), kind="stable")]   # rows of the working file
d = d.take(rows).reset_index(drop=True)          # one copy, in the final order
d["in_sample"] = in_sample[rows].astype(int)
...
# simple fixed-item possessions score: mean of the 16 items, each standardized over the pooled file
...
save_table(d, "base")
# replicate weights in the order of the working file (column 0: final weight), copied from the extracts
# to a memory-mapped file in blocks so that the full matrix is never held in memory
w22 = np.load(C.EXTRACT / "wts_2022.npy", mmap_mode="r")
w25 = np.load(C.EXTRACT / "wts_2025.npy", mmap_mode="r")
W = np.lib.format.open_memmap(C.INTERIM / "base_weights.npy", mode="w+", dtype="float64", shape=(len(d), 1 + C.N_REP))
W[:, 0] = d.w_fstuwt.to_numpy("float64")
for start in range(0, len(rows), 50_000):
    blk = rows[start:start + 50_000]
    m22 = blk < n22
    out = np.empty((len(blk), C.N_REP))
    out[m22] = w22[blk[m22]]
    out[~m22] = w25[blk[~m22] - n22]
    W[start:start + len(blk), 1:] = out
W.flush()
del W
