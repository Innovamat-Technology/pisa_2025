...
Mean-change SEs include the OECD link error; it cancels in changes of group differences.
Usage: python code/python/scripts/05_quartile_changes.py [math|read|scie]
"""
import itertools
import sys
...
W_ALL = np.load(C.INTERIM / "base_weights.npy", mmap_mode="r")   # rows are copied group by group
...
for idx in INDICES + ["escs_cc", "escs_h_cc"]:
    ...
    for cnt in cnts + ["OECD"]:
        ...
        for k in range(4):
            ...
            r[f"Q{k + 1}_change"], r[f"Q{k + 1}_se"] = e25[k] - e22[k], float(brr.mean_change_se(s22[k], s25[k], DOM))
        for i, j in PAIRS:
            c = np.zeros(4)
            c[j], c[i] = 1, -1
...
for idx in sorted({k[2] for k in TH10}):
    ...
    for k in range(10):
        rows.append(dict(index=idx, decile=k + 1, n_systems=len(cnts), mean_2022=e22[k], mean_2025=e25[k],
                         change=e25[k] - e22[k], se=float(brr.mean_change_se(s22[k], s25[k], DOM))))
