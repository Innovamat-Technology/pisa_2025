...
Mean-change SEs include the OECD link error; it cancels in changes of top-minus-bottom gaps.
Usage: python code/python/scripts/11_missing_and_thresholds.py [math|read|scie]
"""
import sys
...
for name in GROUPS:
    ...
    rows.append(dict(group=name, n_systems=len(cnts), pct_2022=sh[2022], pct_2025=sh[2025], mean_2022=e22[0],
                     mean_2025=e25[0], change=e25[0] - e22[0], se=float(brr.mean_change_se(s22[0], s25[0], DOM))))
t = pd.DataFrame(rows)
