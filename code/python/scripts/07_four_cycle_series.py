"""Step 7. Level of the Q4 - Q1 gap by index and cycle, 2015-2025, published and harmonized indices.

The harmonized four-cycle indices use the thirteen possessions items whose concept exists in every
cycle, recoded to common response categories, with one item response model (2PL/GPCM, WLE scores) for the pooled cycles;
HISEI standardized over the pool; parental education on the common years-of-schooling scale
...
standardization, equal-weight mean, re-standardization. Two variants are kept: the mean of the
...
# systems present in every cycle, from the country column alone (the files are read once more below,
# restricted to those systems and to the columns used, to keep memory down)
common = set.intersection(*[set(load_table(f"stu_{y}", ["cnt"]).cnt) for y in YEARS]) - set(C.EXCLUDE)
parts, rows = [], []
for y in YEARS:
    cols = ["wave", "cnt", "cntstuid", "w_fstuwt", "escs", "homepos", "hisei", "paredint"] + PV
    if y <= 2018:
        cols.append("hisced")
    src = sorted({v for old, new, _ in ITEMS13.values() for v in ((old,) if y <= 2018 else (new if isinstance(new, tuple) else (new,)))})
    g = load_table(f"stu_{y}", cols + src)
    keep = g.cnt.isin(common).to_numpy()
    g = g[keep].reset_index(drop=True)
    parts.append(pd.concat([g[cols], recode(g, y)], axis=1))
    rows.append(np.flatnonzero(keep))
d = pd.concat(parts, ignore_index=True)
del parts, g
d["src_row"] = np.concatenate(rows)               # row of each student in the extract of its cycle
WTS = {y: np.load(C.EXTRACT / f"wts_{y}.npy", mmap_mode="r") for y in YEARS}


def weights(g):
    """(n, 81) final weight and replicate weights of the rows of one country-by-cycle group."""
    out = np.empty((len(g), 1 + C.N_REP))
    out[:, 0] = g.w_fstuwt.to_numpy("float64")
    out[:, 1:] = WTS[int(g.wave.iloc[0])][g.src_row.to_numpy()]
    return out
...
# possessions: one item response model for all cycles
names = list(ITEMS13)
...
w = cell_weights(d)
theta, A, B = irt.fit_and_score(X, w / w.sum() * len(d), min_items=8)
pd.DataFrame({"item": names, "a": np.round(A, 3), "steps": [np.round(b, 3).tolist() for b in B]}).to_csv(
    C.TABLES / "irt_parameters_four_cycles.csv", index=False)
d["homepos_h4"] = zscore(theta, w)
for y in YEARS:
    g = d[d.wave == y]
...
RAW = np.column_stack([d.hisei.to_numpy("float64"), d.pared_c.to_numpy("float64"), theta])
...
INDICES = {"escs": "ESCS, published", "escs_h4": "ESCS, harmonized (four cycles)",
           "escs_mean4": "ESCS, harmonized (four cycles), no imputation",
           "escs_pca4": "ESCS, harmonized (four cycles), principal component", "homepos": "HOMEPOS, published",
           "homepos_h4": "HOMEPOS, harmonized (four cycles)", "hisei": "HISEI", "pared_c": "PARED (common scale)",
...
for dom in C.DOMAINS:
    ...
    for (cnt, wave), g in d.groupby(["cnt", "wave"], sort=True):
        P, W = g[PVc].to_numpy("float64"), weights(g)
        ok = np.isfinite(P).all(axis=1)
        for idx in INDICES:
            m = ok & g[idx].notna().to_numpy()
            if m.sum() >= 200:
                TH[(idx, wave, cnt)] = brr.theta_quartiles(g[idx].to_numpy("float64")[m], W[m], P[m])
    rows = []
