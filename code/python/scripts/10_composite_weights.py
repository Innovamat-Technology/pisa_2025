"""Step 10. Ten constructions of the harmonized composite (Table D.1).

1-5 apply the OECD aggregation (equal weights of the standardized components) with different
treatments of missing components and of the standardization scope; 6-10 are principal-component
variants. Construction 1 is the paper's composite (escs_h of step 4).
Usage: python code/python/scripts/10_composite_weights.py [math|read|scie]
"""
import sys
...
d["homepos_irt"] = load_table("homepos_irt", ["homepos_irt"]).homepos_irt.to_numpy()[:len(d)]
w = cell_weights(d)
...
RAW = np.column_stack([d.hisei.to_numpy("float64"), d.paredint.replace({3.0: 6.0, 14.5: 14.0}).to_numpy("float64"),
                       d.homepos_irt.to_numpy("float64")])
...
V["6. First principal component, pooled standardization"] = pca(Xp, every)[0]
s = np.full(len(d), np.nan)
...
for y, m in waves.items():
    si, vs[y] = pca(Xp, m)
    s[m] = si[m]
V["8. Pooled standardization, per-cycle principal component"] = s
