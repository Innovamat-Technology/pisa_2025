"""Step 4. Harmonized components and composites, standardized over the pooled 2022 + 2025 sample.

HISEI  : same ISEI scale in both cycles, only standardized.
PARED  : 3 years -> 6 and 14.5 -> 14 (values that do not exist in 2025), then standardized.
HOMEPOS: WLE score of the common-item response model (step 3).
Books  : ST255, seven identical categories in both cycles.
Composite (escs_h): the OECD rule (escs_rule.py) applied to the three harmonized components in
their original metric, with the two cycles pooled: regression imputation of a single missing
component within system and cycle, senate-weight standardization, equal-weight mean,
re-standardization. escs_pca, the first principal component, is kept as a robustness check
...
g = load_table("homepos_irt").iloc[:len(d)]
...
d["homepos_h"] = zscore(g.homepos_irt, w)
...
RAW = np.column_stack([d.hisei.to_numpy("float64"), pared_c, g.homepos_irt.to_numpy("float64")])
