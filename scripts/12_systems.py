...
A. Q1 and Q4 changes by system under the published and harmonised ESCS and HOMEPOS
B. across systems: correlation of the change in the Q4 - Q1 gap (published, harmonised, and their
   difference) with log GDP per capita, the mean score in 2022 and the mean ESCS in 2022
Usage: python scripts/12_systems.py [math|read|scie]
"""
import sys
...
s["published"], s["harmonised"] = wide[("Q4-Q1_change", "escs")], wide[("Q4-Q1_change", "escs_h")]
s["mismatch"] = s.published - s.harmonised
s.to_csv(C.TABLES / f"systems_income_{DOM}.csv")
r = s.corr().loc[["mismatch", "published", "harmonised"], ["log_gdp_pc", "mean_score_2022", "mean_escs_2022"]]
