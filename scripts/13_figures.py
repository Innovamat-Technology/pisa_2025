...
One colour per version of the indices across every figure: blue for the indices as published by the OECD, orange
for the harmonized ones. The line style tells the index apart: solid for ESCS, dashed for HOMEPOS. HISEI and PARED,
which have no published counterpart, are grey. Uncertainty is shown as 95% error bars (1.96 s.e.).
"""
import matplotlib
matplotlib.use("Agg")
...
for _f in (C.ROOT / "scripts" / "lib" / "fonts").glob("SourceSans3-*.ttf"):
...
def style(idx):
    """Line and marker style of an index: colour by version, line style by index, grey for the single components."""
...
def label(ax, idx, xy):
    """Direct label at the right end of a line: the name of the index, in the colour of the line."""
    grey = COL[idx] == GREY
...
for dom, dname in (("math", "mathematics"), ("scie", "science"), ("read", "reading")):
    ...
    vals = q.loc[SER, [f"Q{k}_change" for k in range(1, 5)]]
    lo, hi = np.floor((vals.min().min() - 4) / 5) * 5, np.ceil((vals.max().max() + 4) / 5) * 5
