...
PISA 2025 Technical Report, chapter 22, "Computation of ESCS" (identical to PISA 2022):
  ...
  2. Each component (imputations included) is standardised to mean 0 and SD 1 with every
     country weighing the same (senate weights).
  3. The three standardised components are averaged with equal weights.
  4. The average is standardised again.
The only departure from the OECD is the scope of the standardisation: here the pool is all the
cycles analysed together, so that the metric is not re-anchored in each cycle.
"""
import numpy as np
...
def escs(X, group, w, imputation="regression", noise=True, seed=SEED):
    """ESCS by the OECD rule. Returns (score, standardised components, share of students imputed).

    imputation: "regression" (the OECD rule), "mean" (mean of the available standardised
